...
function fullName(arr) {
  let result = arr.map((arr) => `${arr.firstName} ${arr.lastName}`);
...
function gradesAvg(arr) {
  var results = arr.map((std) => {
    std.avg =
      std.gradsList.reduce((acc, c) => acc + c, 0) / std.gradsList.length;
    return std;
  });
  return results;
}
